...
public class SocketServer {
	...
	public byte[] FillRetData(byte[] revdata,byte bret,byte[] indata,int insize){
		...
		snddata[0]=(byte)0x46;				//����ͷ
		snddata[1]=(byte)0x43;
		snddata[2]=(byte)revdata[2];		//�豸��
		snddata[3]=(byte)revdata[3];		//����
		snddata[4]=(byte)bret;				//����
		snddata[5]=(byte)(insize&0xFF);		//�������ݳ���
		...
		switch((byte)revdata[CMDIDPOS]){
            ...
            */
			...
			case ZCMD_ADJUSTTIME:{
				...
				ActivityList.getInstance().SetCurrentTime(wYear,cMonth,cDate,cHour,cMin,cSec);
				return FillRetData(revdata,ALLPKG,null,0);
			}
